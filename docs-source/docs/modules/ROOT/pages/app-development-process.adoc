...
Cloudflow includes backend `Streamlet` implementations for Akka Streams, Apache Spark Structured Streaming, and Apache Flink. With the Akka Streams and Apache Flink Streamlet implementations you can write business logic in the native API of the backend, using either Java or Scala. Additionally, Cloudflow can be extended with new streaming backends.
